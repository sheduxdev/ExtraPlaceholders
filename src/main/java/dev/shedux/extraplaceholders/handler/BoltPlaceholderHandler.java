 ...
 * <ul>
 *   <li>Match winner/loser tracking for all match types (solo, team, FFA)</li>
 *   <li>Current kit rule checking</li>
 *   <li>Specific kit rule validation</li>
 * </ul>
 *
 * @author sheduxdev
 * @since 1.0.0
 */
public class BoltPlaceholderHandler implements PlaceholderHandler {
    ...
    private static final Map<String, Function<IKit, Boolean>> KIT_RULES = createKitRulesMap();
    ...
    private String handleMatchPlaceholder(OfflinePlayer player, List<String> args, BoltTracker tracker) {
        if (args.size() < 3 || !(player instanceof Player onlinePlayer)) {
            return Configuration.MESSAGES.BOLT_NOT_AVAILABLE;
        }

        return switch (args.get(2).toLowerCase()) {
    ...
    private String handleKitPlaceholder(OfflinePlayer player, List<String> args, BoltTracker tracker) {
        ...
        IMatch match = tracker.getApi().getMatchAPI().getMatchByPlayer(onlinePlayer);
        if (match == null) {
        ...
        if (args.size() > 2 && RULE_COMMAND.equalsIgnoreCase(args.get(2))) {
            return handleCurrentKitRule(args, currentKit);
        }

        if (args.size() >= 4 && RULE_COMMAND.equalsIgnoreCase(args.get(3))) {
            return handleSpecificKitRule(args.get(2), args, tracker);
        }
    ...
    /**
     * Generic method to get match results based on a resolver strategy.
     */
    private String getMatchResult(Player player, BoltTracker tracker, MatchResultResolver resolver) {
        IMatch match = tracker.getApi().getMatchAPI().getMatchByPlayer(player);
        ...
        if (match.isSoloMatch()) {
            return resolver.resolveSolo(match).map(this::getPlayerName)
                    .orElse(Configuration.MESSAGES.BOLT_NOT_AVAILABLE);
        }

        if (match.isTeamMatch()) {
            return resolver.resolveTeam(match).map(this::formatTeamNames)
                    .orElse(Configuration.MESSAGES.BOLT_NOT_AVAILABLE);
        }
    ...
    private String handleCurrentKitRule(List<String> args, IKit currentKit) {
        ...
        return checkKitRule(currentKit, args.get(3).toLowerCase()) ? TRUE : FALSE;
    }

    /**
     * Handles specific kit rule checking.
     */
    private String handleSpecificKitRule(String kitName, List<String> args, BoltTracker tracker) {
        if (args.size() < 5) {
            return null;
        }

        IKit kit = tracker.getApi().getKitAPI().getKit(kitName);
        if (kit == null) {
            return Configuration.MESSAGES.KIT_INVALID;
        }

        return checkKitRule(kit, args.get(4).toLowerCase()) ? TRUE : FALSE;
    }
    ...
    private boolean checkKitRule(IKit kit, String ruleName) {
        return Optional.ofNullable(KIT_RULES.get(ruleName))
    ...
    private Optional<IMatchTeam> findWinnerTeam(IMatch match) {
        ...
        if (teams.size() != 2) {
            return teams.isEmpty() ? Optional.empty() : Optional.of(teams.getFirst());
        }
